...
def get_open_price(candles):
    """First candle open = market open price."""
    if not candles:
        return None, None
    return candles[0]['open'], candles[0]['time']

def get_second_candle_price(candles):
    """Second candle open = 6:32 AM PST hold price."""
    if len(candles) < 2:
...
def minutes_to_candle(candles, target_time_str):
    """Estimate minutes elapsed from open to a given candle time string."""
    if not candles or not target_time_str:
        return 0
    try:
        open_time   = candles[0]['time']
        open_dt     = datetime.fromisoformat(open_time.split('+')[0].split('-0')[0])
        target_dt   = datetime.fromisoformat(target_time_str.split('+')[0].split('-0')[0])
        delta_mins  = (target_dt - open_dt).total_seconds() / 60
        return max(0, int(delta_mins))
    except Exception:
        return 0

def calc_fomo(trade, candles, sell_price, sell_time):
    """Run Black-Scholes FOMO calculation for day high and low."""
    entry      = trade['entry']
    ...
    if day_high and day_low:
        ...
        # For a CALL: high is best. For a PUT: low is best.
        if option_type == 'CALL':
            best_value  = contract_at_high
            worst_value = contract_at_low
            best_time   = high_time
            worst_time  = low_time
        else:
            best_value  = contract_at_low
            worst_value = contract_at_high
            best_time   = low_time
            worst_time  = high_time
...
def update_stats(stats, trade):
    ...
    if won: d['wins'] += 1
    d['avg_pnl'] = round(
        (d['avg_pnl'] * (d['trades'] - 1) + pnl) / d['trades'], 2
    )
    ...
    if fomo:
        ...
        f = stats['fomo']
        total_fomo_trades = o['total_trades']
        f['avg_left_on_table'] = round(
            (f['avg_left_on_table'] * (total_fomo_trades - 1) + left) / total_fomo_trades, 2
        )
...
def update_blog(blog, trade):
    """Add full trade data to the blog entry skeleton."""
    entry  = trade['entry']
    exit_  = trade['exit']
    fomo   = trade['fomo']
    news   = trade['news_scan']
    ...
    if fomo and fomo.get('left_on_table') is not None:
        left = fomo['left_on_table']
        left_str = f" Left on table: ${left:+.2f}."
...
def main():
    ...
    for trade in open_trades:
        ...
        # Select sell candle based on news decision
        if decision == 'hold':
        ...
        # Stock open price → calculate option value via Black-Scholes
        ...
        # FOMO calculation
        print(f"  Running Black-Scholes FOMO...")
        fomo = calc_fomo(trade, candles, sell_price, sell_time)
        if fomo.get('best_case'):
            print(f"  Best case: ${fomo['best_case']['pnl']:+.2f} | "
                  f"Left on table: ${fomo.get('left_on_table', 0):+.2f}")
        ...
        # Update running balance (cumulative P&L)
        trades_data['balance'] = round(trades_data['balance'] + pnl, 2)

        # Update stats and blog
        update_stats(stats, trade)
        update_blog(blog, trade)

        print(f"  Trade #{trade['id']} closed. Running P&L: ${trades_data['balance']:+.2f}")
